...
import { doc, setDoc, updateDoc, increment, getDoc, arrayUnion, collection, query, where, getDocs } from "firebase/firestore";
...
const RegistrationForm = () => {
    ...
    // ✅ Find Referrer User by Referral Code (UPDATED)
    const findReferrerByCode = async (referralCode) => {
        try {
            // Query the User collection where ReferCode matches the provided referral code
            const usersRef = collection(db, "User");
            const q = query(usersRef, where("ReferCode", "==", referralCode));
            const querySnapshot = await getDocs(q);

            if (!querySnapshot.empty) {
                // Get the first matching user (assuming referral codes are unique)
                const referrerDoc = querySnapshot.docs[0];
                return {
                    uid: referrerDoc.id, // This is the Firebase Auth UID (document ID)
                    ...referrerDoc.data()
                };
            }
            return null;
    ...
    // ✅ Add Referral to Referrer's Referral Subcollection (UPDATED)


    const addReferralToReferrer = async (referrerUID, newUserName, newUserUID, newUserAuthUID) => {
        try {
            const referralRef = doc(db, "User", referrerUID, "Refferal", "docId");
            const timestamp = getPreciseTimestamp();

            const referralData = {
                Amount: 0, // Setting initial amount as 0 as per your requirement
                CreatedAt: timestamp,
                Name: newUserName,
            ...
            // First check if the referral document exists
            const referralDoc = await getDoc(referralRef);

            if (referralDoc.exists()) {
                // If document exists, update it by adding to the referrals array
                await updateDoc(referralRef, {
                    refferals: arrayUnion(referralData)
                });
                console.log("✅ Referral added to existing referrals array");
            } else {
                // If document doesn't exist, create it with the referrals array
                await setDoc(referralRef, {
                    refferals: [referralData]
                });
                console.log("✅ Referral document created and data added");
            }

        } catch (error) {
            console.error("❌ Error in addReferralToReferrer:", error);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // ✅ Check if referral code exists and find referrer (UPDATED)
            let referrerData = null;
            if (formData.referral && formData.referral.trim() !== "") {
                referrerData = await findReferrerByCode(formData.referral);
                if (referrerData) {
                    console.log("✅ Referrer found:", referrerData.Name, "with ReferCode:", referrerData.ReferCode);
                } else {
                    console.log("❌ No user found with this referral code:", formData.referral);
                }
            }
            ...
            const userData = {
                ...
                Token: "",
                isBlocked: false,
                // Add ReferCode for the new user (you might want to generate one for them too)
                ReferCode: "",
            };

            await setDoc(doc(db, "User", uid), userData);

            // ✅ Add referral to referrer's Referral subcollection if referral code was used and found
            if (referrerData) {
                await addReferralToReferrer(referrerData.uid, formData.name, userUID, uid);
                console.log("✅ Referral successfully recorded for:", referrerData.Name);
            }
